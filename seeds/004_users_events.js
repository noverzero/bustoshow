...
const userId = () => { return util.random.int(1,10)}
const eventId = () => { return util.random.int(1,18)}

const generateUsersEventsSeeds = (num) => {
  let usersEvents = []
  for (let i = 0; i < num; i++) {
    usersEvents.push(
...
exports.seed = (knex) => {
  ...
  return knex('users_events').del()
    .then(() => {
      ...
      return knex('users_events').insert(generateUsersEventsSeeds(50))
    })
    .then(() => {
